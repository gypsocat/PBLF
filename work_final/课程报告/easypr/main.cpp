...
int main(){
    ...
    pr.setDetectType(PR_DETECT_COLOR | PR_DETECT_SOBEL|PR_DETECT_CMSER);
	  ...
	  pr.setResultShow(	true	);
    pr.setMaxPlates(10);
						//TODO：我们首先定义一个pr变量，设置识别时的参数。

	vector<CPlate> plateVec;
    					//我们定义了一个plateVec的顺序表
	Mat src = imread(		"EasyPR-master/resources/image/native_test/3.jpg");
						//TODO：图片的地址，注意地址需要用引号引用
	int result = pr.plateRecognize(src, plateVec);
	 					//识别结果返回给result
    
	if (	result==0		)   //TODO：什么才表示识别成功了呢？
	{
        
        
		int total=  plateVec.size() ;//TODO：一共实际识别到多少个车牌
        				
		
		for(i=1;	i<=	plateVec.size();i++)
						//TODO：利用循环逐个输出被识别到的车牌。
		{
		...
		//TODO：你需要一个输出语句。
    std::cout << "识别第"<<i<<"个车牌为: " << license << endl;
